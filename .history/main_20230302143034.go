...
func req(paramStr ) model.ResData {
	res := model.Res{}
...
func handleStockDailyData(data model.ResData) (s []model.StockDailyData) {
	for _, v := range data.Items {
		...
		s = append(s, model.StockDailyData{
			Ts_Code: ts_code,
			Trade_Date : trade_date,omitempty"`
	Open      : open,omitempty"`
	High      : high,omitempty"`
	Low       : low,omitempty"`
	Close     : close,omitempty"`
	Pre_Close : pre_close,omitempty"`
	Change    : change,omitempty"`
	Pct_Chg   : pct_chg,omitempty"`
	Vol       : vol,omitempty"`
