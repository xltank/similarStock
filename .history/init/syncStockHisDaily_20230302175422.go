...
func main() {
	...
	for d := startDate; d.Before(endDate); d.AddDate(0, 0, 1) {
		p := strings.Replace(allStockDailyParam, "$date", d.Format("20060102"), -1)
		stockDaily := req(p)
		fmt.Println("daily data length:", d.Format("20060102"), len(stockDaily.Items))
